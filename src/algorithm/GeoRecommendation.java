...
public class GeoRecommendation implements Recommendation {
	...
	public List<Item> recommendItems(String userId, double lat, double lon) {
		...
		// Step 1: fetch all the events (ids) this user has visited.
		Set<String> favoriteItems = conn.getFavoriteItemIds(userId); // db quries
		
		// Step 2: Get categories of favoriteItems
		Set<String> allCategories = new HashSet<>();
		for (String item : favoriteItems) {
			allCategories.addAll(conn.getCategories(item)); // db queries
		}
		
		// Step 3: Find events that belong to these categories
		...
		// Step 4: get events that this user has not visited
		...
		Collections.sort(filteredItems, new Comparator<Item>() {
			...
			public int compare(Item item1, Item item2) {
				...
				if (Math.abs(distance1 - distance2) < 0.0000001) {
					return 0;
				}
	...
	// Helper: Calculate the distances between two geo locations.
